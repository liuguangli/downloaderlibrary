        ...
        try {
            ...
            while ((length = is.read(buffer)) != -1) {
                ...
                if (item.state == FileInfo.FileItem.COMPLETED) {
                    ...
                    Log.d(TAG, "Thread " + getId() + "  完成一个ApkItem " + item._id + " lengthL:" + length);
                }
            ...
            Log.d(TAG, "Item" + item._id + " 实际下载长度:" + totalLength + " 应下载长度：" + needLength);
            if (totalLength > needLength)
                Log.e(TAG, "Item 实际下载长度:" + totalLength + " 应下载长度：" + needLength);
            if (totalLength < needLength) {
                downloadApk(item);
                Log.e(TAG, "Thread -" + getId() + " 网络没请求到实际长度的内容内容，重新下载");
            }
        } catch (EOFException e) {
            downloadApk(item);
            Log.e(TAG, "Thread -" + getId() + " EOFExcetion,数据意外中断，重新下载");
        } catch (Exception e) {
            e.printStackTrace();
            ...
            if (!TextUtils.isEmpty(e.getMessage())) {
                if (msgString.contains("Connection reset by peer")) {
                    downloadApk(item);
                    Log.e(TAG, "Thread -" + getId() + " Connection reset by peer, 服务器重置，数据意外断开，重新下载");
                    return;
                }
